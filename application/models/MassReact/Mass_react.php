...
class Mass_react extends CI_Model {
  ...
  public function index()
  {
    ...
    $input = $this->climate->br()->input(' เลื่อก :');
    $input = $input->prompt();
    switch ($input)
    {
      case '01':
  ...
  private function input_home()
  {
    ...
    {
      $this->climate->br()->shout('  ไม่พบ input');
      sleep(3);
      $this->tools->mass_react('Mass React');
    }
  }
  ...
  {
    $input = $this->climate->br()->input(' ใส่UIDหรือUSerเหยื่อ:');
  ...
  {
    $this->climate->br()->info('  คุณต้องเข้าร่วมกลุ่มเป้าหมาย')->br();
    $input = $this->climate->input('  ใสIDกลุ่ม:');
    $id = $input->prompt();
    if (!is_numeric($id))
    {
      $this->climate->br()->shout(' ไม่พบ id');
      sleep(3);
      $this->tools->mass_react('Mass React');
    }
    $this->url = $this->base_url.'/groups/'.$id;
    $input = $this->climate->input('  สูงสุด (แนะนำ: 90):');
    $this->limit = $input->prompt();
    if (!is_numeric($this->limit) OR $this->limit == '0')
    {
      $this->climate->br()->shout('  ไม่พบ input');
  ...
  private function input_fanspage()
  {
    ...
    if (!trim($username))
    {
      $this->climate->br()->shout(' ไม่พบUser');
      sleep(3);
      $this->tools->mass_react('Mass React');
    }
  ...
  private function dump_post()
  {
    ...
    if (!$stop)
    {
      ...
      {
        $hrefs = $href->getAttribute('href');
        if (strpos($href->nodeValue, 'ดูอย่างอื่น') !== false)
        {
          $this->url = $this->base_url.$hrefs;
          $this->dump_post();
          break;
        }
        else if (strpos($href->nodeValue, 'เพิ่มเติม') !== false)
        {
          $this->url = $this->base_url.$hrefs;
          $this->dump_post();
          break;
        }
        else if (strpos($href->nodeValue, 'ดูอย่างอื่นเพิ่ม') !== false)
        {
          $this->url = $this->base_url.$hrefs;
          $this->dump_post();
          break;
    ...
    {
      $this->climate->br()->br()->shout('  เริ่มทำงาน...')->br();
      foreach ($this->data as $post)
      {
        $this->execute($post);
      }
      $this->climate->br()->out('  เสร็จสิ้นขอบคุณที่ใช้เครื่องมือนี้');
      $this->configs->back_menu();
    }
    else
    {
      $this->climate->shout('  ไม่พบโพสต์');
      $this->configs->back_menu();
    }
  }
