...
class Mass_comments extends CI_Model {
  ...
  {
    $input = $this->climate->br()->input('  ไม่พบUIDหรือU:');
    $username = $input->prompt();
    if (!trim($username))
    {
      $this->climate->br()->shout('  กรุณาเช็คดีๆ');
      sleep(3);
      $this->tools->mass_comments('Mass Comments');
    }
    $this->url = $this->base_url.'/'.$username.'?v=timeline';
    $this->climate->br()->shout("  ใส่ข้อความเว้นวรรค ใส่'<n>' ตัวอย่าง : hello<n>world")->br();
    $input = $this->climate->input('  ข้อความ:');
    $this->msg = $input->prompt();
    if (!trim($this->msg))
    {
    ...
    $input = $this->climate->input('  สูงสุ (แนะนำ: 90):');
    $this->limit = $input->prompt();
    if (!is_numeric($this->limit) OR $this->limit == '0')
    {
      $this->climate->br()->shout(' ไม่พบตัวส่ง');
      sleep(3);
      $this->tools->mass_comments('Mass Comments');
    }
  }
  ...
  {
    $this->climate->br()->info(' คุณยังไม่อยู่ในกลุ่มเป้าหมาย')->br();
    $input = $this->climate->input('  ใส่IDกลุ่ม:');
    $id = $input->prompt();
    if (!is_numeric($id))
    {
      $this->climate->br()->shout('  กรุณาเช็คดีๆ');
  ...
  {
    $input = $this->climate->br()->input(' ไม่พบUIDหรือuser:');
    $username = $input->prompt();
    if (!trim($username))
    {
      $this->climate->br()->shout('  ไม่พบ');
    ...
    $this->climate->br()->shout("  ตัวอย่าง <n> Welcome")->br();
    $input = $this->climate->input('  ข้อความ:');
    $this->msg = $input->prompt();
    if (!trim($this->msg))
    {
