...
class Auth extends CI_Model {
  ...
  public function login()
  {
    ...
    $this->climate->info('  (?) พิม C เพื่อล็อคอินด้วย cookies, พิม U ล็อคอินด้วย username and password')->br();
  ...
  private function login_cookies()
  {
    ...
    */
    while (TRUE)
    {
      $input = $this->climate->input('  ใส่Cookies :');
      $cookie = $input->prompt();
      /*
        @Create Directory
      */
      ...
      if (strpos($response, 'mbasic_logout_button') !== false)
      {
        ...
        fwrite($file, $cookie);
        fclose($file);
        $this->climate->br()->info(" ล็อคอินเรียบร้อย , สวัสดี ${name}")->br();
        sleep(3);
      ...
      {
        $this->climate->br()->shout('  Cookies ตาย')->br();
        continue;
      }
    }
  }
  ...
  private function change_language($cookies)
  {
    ...
    {
      if (strpos($href->nodeValue, 'ไทย') !== false)
      {
  ...
  private function login_userpass()
  {
    ...
    while (TRUE)
    {
      ...
      */
      ...
      {
        $this->climate->br()->shout('  ลองไหม่')->br();
      }
    }
  }
}