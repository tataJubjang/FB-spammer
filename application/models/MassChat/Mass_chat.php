...
class Mass_chat extends CI_Model {
  ...
  {
    $this->climate->shout("  ใส่ '<n>' เพื่อเว้นวรรค ตัวอย่าง: hello<n>world")->br();
    $input = $this->climate->input('  ข้อความ:');
    $this->messages = $input->prompt();
    if (!trim($this->messages))
    {
      $this->climate->br()->shout("  เอาไหม่");
      sleep(3);
      $this->tools->mass_chat('รายการแชทเป็นกลุ่ม');
      exit(0);
    }
  ...
  public function index()
  {
      ...
      if (strpos($style, 'vertical-align: middle') !== false)
      {
        ...
        foreach ($find as $href)
        {
          ...
          if ($id[1] AND $name)
          {
            ...
            {
              $this->data[] = $id[1];
              $this->climate->out('  - ส่งไปที่: '.$name);
              $this->chats();
            }
          }
        }
    ...
    if (!$stop)
    {
      ...
      {
        $hrefs = $href->getAttribute('href');
        if (strpos($href->nodeValue, 'ดูเพิ่มเติม') !== false)
        {
    ...
    {
      $this->climate->shout('  ไม่พบเพื่อน');
