...
class App extends CI_Controller {
  ...
  */
  ...
  private function about_tools()
  {
    ...
    $this->climate->out('  Author          : SCK_TH');
    ...
    $this->climate->out('  Language        : TH');
    $this->climate->out("                    * Facebook ({$this->green}https://web.facebook.com/Talovejub.270361{$this->reset})");
    $this->climate->out("                    * Youtube ({$this->green}https://www.youtube.com/channel/UCAgvG9YzURgLGGeEivig4vg?view_as=subscriber{$this->reset})");
    $this->configs->back_menu();
  }

  private function _curl_version()
  {
    $response = file_get_contents('https://raw.githubusercontent.com/tataJubjang/FB-spammer/master/version.txt');
  ...
  private function update()
  {
    ...
    {
      $this->climate->br()->shout('  หมดอายุ');
      $this->configs->back_menu();
    }
    else
    {
      $this->climate->br()->info('  กำลังอัพเดต...')->br();
  ...
  public function menu()
  {
    $this->configs->clear();
    $date = date('D M j G:i:s Y');
    echo sprintf($this->configs->banner(), $date, 'เช็ค cookies...');
    ...
    if (strpos($response, 'mbasic_logout_button') === FALSE)
    {
      ...
      unlink('log/cookies.txt');
      $this->climate->shout('  [WARNING] Cookies ไม่พบหรือหมดอายุ.');
    ...
    if ($version)
    {
      $this->climate->info(" เวอร์ชั่นทดลอง กรถณาใช้ดีๆ");
    }
    $this->climate->out('  +------------------------------------------------------+');
    $this->climate->out("    [{$this->yellow}RC{$this->reset}].    ลบ Cookies");
    $this->climate->out("    [{$this->yellow}RB{$this->reset}]. Report Bug");
    $this->climate->out("    [{$this->yellow}UT{$this->reset}]. อัพเดต Tools");
    $this->climate->out("    [{$this->yellow}SC{$this->reset}]. ด Cookies");
    $this->climate->out("    [{$this->yellow}AT{$this->reset}]. ข้อมูล");
    ...
    switch (strtolower($input))
    {
      ...
      case 'ut':
        ...
        break;
      case 'ex':
        $this->climate->br()->yellow("  ขอบคุณที่มาเป็นส่วนหนึ่งของSCK :)");
        exit(0);
      case 'sc':
        $this->see_cookies();
        break;
